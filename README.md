...
```bash
chmod +x rocketchat-mattermost-*       # Linux/macOS
./rocketchat-mattermost-* --help
```
...
- **Source side** (`export` + `package`): either directly on the RocketChat
  ...
  MONGO_URI="mongodb://localhost:27018/rocketchat" ./rocketchat-mattermost export
  ```
...
Examples use the binary; from source, replace `./rocketchat-mattermost` with
`pnpm cli`.
...
```bash
./rocketchat-mattermost package      # → mattermost-import.zip
```
...
```bash
./rocketchat-mattermost reset-password
```
...
```bash
ARCHIVE_CHANNELS=true READ_STATE=read ./rocketchat-mattermost export
```

## Building a binary

```bash
pnpm build:binary        # → dist/rocketchat-mattermost (current platform)
```
